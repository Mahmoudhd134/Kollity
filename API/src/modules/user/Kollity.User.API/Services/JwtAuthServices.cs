...
public class JwtAuthServices : IAuthServices
{
    ...
    public JwtAuthServices(UserDbContext context,
    ...
    public async Task<Result<RefreshTokenDto>> LoginUser(LoginUserDto loginUserDto, string deviceToken)
    {
        ...
        var oldRefreshToken = await _context.UserRefreshTokens
            .FirstOrDefaultAsync(urt => urt.UserId == user.Id && urt.DeviceToken == deviceToken);
    ...
    public async Task<Result<RefreshTokenDto>> RefreshToken(Guid userId, string refreshToken, string deviceToken)
    {
        ...
        var userRefreshToken = await _context.UserRefreshTokens
            .Include(u => u.User)
            .FirstOrDefaultAsync(u => u.UserId == userId && u.DeviceToken == deviceToken);
