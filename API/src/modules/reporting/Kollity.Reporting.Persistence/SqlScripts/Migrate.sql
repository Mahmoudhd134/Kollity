...
begin
    ...
    begin transaction migrate_data
        begin try
            ...
            -- migrate user data
            insert into KollityFeedbackDB.feedback.[User](id, user_name, full_name, user_type, is_deleted)
            select id,
                   user_name,
                   full_name_in_arabic,
                   case
            ...
            -- migrate course data
            insert into KollityFeedbackDB.feedback.Course(id, department, code, hours, name, is_deleted)
            select id, department, code, hours, name, cast(0 as bit)
            from KollityServicesDb.services.Course

            insert into KollityFeedbackDB.feedback.CourseStudent (id, course_id, student_id)
            select newid(), course_id, student_id
            from KollityServicesDb.services.StudentCourse
            -- migration of course data ended


            --migrate rooms data
            insert into KollityFeedbackDB.feedback.Room (id, name, course_id, doctor_id, is_deleted)
            select id, name, course_id, doctor_id, cast(0 as bit)
            from KollityServicesDb.services.Room;

            insert into KollityFeedbackDB.feedback.RoomUser(room_id, user_id)
            select room_id, user_id
            from KollityServicesDb.services.UserRoom
            where join_request_accepted = 1
            -- migration of rooms data ended
            ...
            -- migrate exam data
            insert into KollityFeedbackDB.feedback.Exam (id, name, room_id, is_deleted)
            select e.id, e.name, e.room_id, cast(0 as bit)
            from KollityServicesDb.services.Exam E
...
where exam_question_id = '92b1f30c-033e-4a51-0b37-08dc3fccf02e'
  and is_right_option = 1
SELECT (SELECT COUNT(*)
        FROM [reporting].[Exam] AS [e]
...
SELECT [t].[id],
       ...
       [t2].[CreatedAtUtc],
       [t2].[id0]
FROM (SELECT TOP (1) [c].[id], [c].[code], [c].[name], [c].[department], [c].[hours]
      FROM [reporting].[Course] AS [c]
      WHERE [c].[id] = @__request_Id_0) AS [t]
         LEFT JOIN (SELECT [c0].[id]                    AS [Id],
                           [t1].[profile_image]         AS [Image],
                           [t1].[full_name_in_arabic]   AS [FullName],
                           CASE
                               WHEN [c0].[is_doctor] = CAST(1 AS bit) THEN 1
                               ELSE 2
                               END                      AS [DoctorType],
                           [c0].[is_currently_assigned] AS [IsCurrentlyAssigned],
                           [c0].[assigning_date]        AS [AssignedAtUtc],
                           [t1].[id]                    AS [id0],
                           [c0].[course_id]
                    FROM [reporting].[CourseDoctorAndAssistant] AS [c0]
                             INNER JOIN (SELECT [u].[id], [u].[full_name_in_arabic], [u].[profile_image]
                                         FROM [reporting].[User] AS [u]
                                         WHERE [u].[type] = N'Doctor') AS [t1] ON [c0].[doctor_id] = [t1].[id]) AS [t0]
                   ON [t].[id] = [t0].[course_id]
         LEFT JOIN (SELECT [r].[id]                   AS [Id],
                           [r].[name]                 AS [Name],
                           [r].[doctor_id]            AS [DoctorId],
                           [t3].[full_name_in_arabic] AS [DoctorName],
                           [r].[created_at]
                                                      AS [CreatedAtUtc],
                           [t3].[id]                  AS [id0],
                           [r].[course_id]
                    FROM [reporting].[Room] AS [r]
                             INNER JOIN (SELECT [u0].[id], [u0].[full_name_in_arabic]
                                         FROM [reporting].[User] AS [u0]
                                         WHERE [u0].[type] = N'Doctor') AS [t3] ON [r].[doctor_id] = [t3].[id]) AS [t2]
                   ON [t].[id] = [t2].[course_id]
...
SELECT [r].[id]                                          AS [Id],
       ...
       (SELECT MAX([t].[c])
        FROM (SELECT (SELECT COALESCE(SUM(CASE
                                              WHEN [e9].[is_right_option] = CAST(1 AS bit)
                                                  THEN CAST([e10].[degree] AS int)
                                              ELSE 0
            END), 0)
                      FROM [reporting].[ExamAnswer] AS [e8]
                               INNER JOIN [reporting].[ExamQuestionOption] AS [e9]
                                          ON [e8].[exam_question_option_id] = [e9].[id]
                               INNER JOIN [reporting].[ExamQuestion] AS [e10] ON [e8].[exam_question_id] = [e10].[id]
                      WHERE [r].[id] = [e8].[room_id]
                        AND [e7].[student_id] = [e8].[student_id]) AS [c],
                     [e7].[student_id]
              FROM [reporting].[ExamAnswer] AS [e7]
              WHERE [r].[id] = [e7].[room_id]
              GROUP BY [e7].[student_id]) AS [t])        AS [MaxSumOfStudentDegree],
       (SELECT MIN([t0].[c])
        FROM (SELECT (SELECT COALESCE(SUM(CASE
                                              WHEN [e13].[is_right_option] = CAST(1 AS bit)
                                                  THEN CAST([e14].[degree] AS int)
                                              ELSE 0
            END), 0)
                      FROM [reporting].[ExamAnswer] AS [e12]
                               INNER JOIN [reporting].[ExamQuestionOption] AS [e13]
                                          ON [e12].[exam_question_option_id] = [e13].[id]
                               INNER JOIN [reporting].[ExamQuestion] AS [e14] ON [e12].[exam_question_id] = [e14].[id]
                      WHERE [r].[id] = [e12].[room_id]
                        AND [e11].[student_id] = [e12].[student_id]) AS [c],
                     [e11].[student_id]
              FROM [reporting].[ExamAnswer] AS [e11]
              WHERE [r].[id] = [e11].[room_id]
              GROUP BY [e11].[student_id]) AS [t0])      AS [MinSumOfStudentDegree],
       (SELECT COUNT(*)
        FROM (SELECT DISTINCT [e15].[student_id]
              FROM [reporting].[ExamAnswer] AS [e15]
              WHERE [r].[id] = [e15].[room_id]) AS [t1]) AS [NumberOfStudentsAnswers]
FROM [reporting].[Room] AS [r]
...
SELECT [r].[id]                                         AS [Id],
       ...
       (SELECT MAX([t].[c])
        FROM (SELECT COALESCE(SUM([a2].[degree]), 0) AS [c], [a2].[student_id]
              FROM [reporting].[AssignmentAnswer] AS [a2]
              WHERE [r].[id] = [a2].[room_id]
              GROUP BY [a2].[student_id]) AS [t])       AS [MaxSumStudentDegree],
       (SELECT MIN([t0].[c])
        FROM (SELECT COALESCE(SUM([a3].[degree]), 0) AS [c], [a3].[student_id]
              FROM [reporting].[AssignmentAnswer] AS [a3]
              WHERE [r].[id] = [a3].[room_id]
              GROUP BY [a3].[student_id]) AS [t0])      AS [MinSumStudentDegree],
       (SELECT COUNT(*)
        FROM (SELECT DISTINCT [a4].[student_id]
              FROM [reporting].[AssignmentAnswer] AS [a4]
              WHERE [r].[id] = [a4].[room_id]) AS [t1]) AS [NumberOfStudentsAnswers]
FROM [reporting].[Room] AS [r]
WHERE [r].[course_id] = @__request_Id_0
