...
public class GetExamNextQuestionCommandHandler : ICommandHandler<GetExamNextQuestionCommand, ExamQuestionForAnswerDto>
{
    ...
    public GetExamNextQuestionCommandHandler(ExamsDbContext context, IUserServices userServices)
    {
        _context = context;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var userInRoom = await _context.RoomUsers
            .AnyAsync(x => x.UserId == userId && x.RoomId == exam.RoomId, cancellationToken);
        if (userInRoom == false)
